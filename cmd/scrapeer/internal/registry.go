...
func (r *Registry) ActivePeers() ([]PeerNode, error) {
	peers, err := r.Peers()
	if err != nil {
		return peers, errors.Wrap(err, "failed to get peers from storage")
	}
	respondingPeers := make([]PeerNode, 0)
	for _, p := range peers {
		if p.State == NodeGreeted || p.State == NodeResponding {
			respondingPeers = append(respondingPeers, p)
		}
	}
	connections := r.Connections()
	return Union(respondingPeers, connections), nil
}
...
func Union(a, b []PeerNode) []PeerNode {
	m := make(map[string]bool)
	for _, item := range a {
		m[item.Address.String()] = true
	}
	for _, item := range b {
		if _, ok := m[item.Address.String()]; !ok {
			a = append(a, item)
		}
	}
	return a
